...
//				Zihao Xing, CSE687-OOD, Project#2, Spring 17 //
///////////////////////////////////////////////////////////////
/*
   Package Operations:
   ===================
   ...
   ---------------
     Graph.h, Graph.cpp, 
	 StrongComponent.h, dependecyTable.h
   ...
   -----------------
   ver 1.7: 8 Mar 17
   - added functionality to initialize from dependency table 
   - added functionality to find strong connected component
   ver 1.6 : 23 Feb 17
...
#include "../DependencyAnalysis/DepencyTable.h"
...
namespace GraphLib 
{
  ...
  public:
    ...
    size_t size();
    bool& mark();
	std::vector< Vertex<V, E> > childNodes_;
  private:
  ...
  public:
    ...
    size_t size();
	void initialize(DependenceAnalysis::DependcyTable deptable);
	// do SCC analysis
	void SCC(StrongComponents& SCCs);
    template<typename F>
  ...
  private:
	...
	void SCCUtil(size_t u, size_t disc[], size_t low[], std::stack<Vertex<V, E>>* st, bool stackMember[], StrongComponents* SCC);
	// init functor for DFS
	class initFunc
	{
	public:
	...
	// find vertext bu vertex's value
	Vertex<V, E> findVert(std::string value) 
	{
		for (auto item : adj) 
		{
  ...
  void Graph<V,E>::addEdge(E eVal, Vertex<V,E>& parent, Vertex<V,E>& child)
  {
	...
	adj[parentIndex].childNodes_.push_back(child);
  }
  ...
  // ---------------<My Stuff>-----------
  // ----<A recursive function that find strongly connected components>----
  template<typename V, typename E>
  void Graph<V, E>::SCCUtil(size_t u, size_t disc[], size_t low[], std::stack<Vertex<V,E>>* st, bool stackMember[], StrongComponents* SCCs)
  {
	  static size_t time = 0;
	  // Initialize discovery time and low value
	  disc[u] = low[u] = ++time;
	  st->push((*this)[(*this).findVertexIndexById(u)]);
	  stackMember[u] = true;
	  // Go through all vertices adjacent to this
	  for (auto& i : (*this)[findVertexIndexById(u)].childNodes_)
	  {   // v is current adjacent of 'u'
		  size_t v = i.id(); // If v is not visited yet, then recur for it
		  if (disc[v] == -1)
		  {
			  SCCUtil(v, disc, low, st, stackMember, SCCs);
			  low[u] = std::min<size_t>(low[u], low[v]);
		  }
		  else if (stackMember[v] == true)
			low[u] = std::min<size_t>(low[u], disc[v]);
	  }
	  ...
	  if (low[u] == disc[u])
	  {
		  ...
		  stackMember[w] = false;
		  st->pop();
		  SCCs->addSC(sc); // find strong components
	  }
  }

  // ----<the function to do DFS traverse, It uses SCCUtil>-----
  template<typename V, typename E>
  void Graph<V, E>::SCC(StrongComponents& SCCs)
  {
	  using namespace std;
	  size_t NUMS = size();
	  size_t *disc = new size_t[NUMS];
	  size_t *low = new size_t[NUMS];
	  bool *stackMember = new bool[NUMS];
  ...
  // ---<this function used to initialize Graph from Dependency Table>----
  template<typename V, typename E>
  void Graph<V, E>::initialize(DependenceAnalysis::DependcyTable deptable)
  {	  
	  std::vector<DependenceAnalysis::TableElem> DataLines = deptable.getTinyDepTable();
	  std::vector<std::string> verts = deptable.getdepfiles();
	  // add vertexs
	  for (auto item : verts) 
	  {
		  this->addVertex(deptable.getFileName(item));
	  }
	  // add edges
	  for (auto edgeitem: DataLines)
	  {
		  for (size_t i = 0; i < edgeitem.relatedFiles.size(); i++)
		  {
			  this->addEdge("e", findVert(deptable.getFileName(edgeitem.files)), findVert(deptable.getFileName(edgeitem.relatedFiles[i])));
		  }
	  }
	  std::cout << "\n Have already initialize graph from dependency table\n===============================================\n";
	  return;
  }
    ...
    static void show(graph& g)
    {
      ...
      while(iter != g.end())
      {
        ...
        for(size_t i=0; i<v.size(); ++i)
        {
          ...
          std::cout << "\n    edge points to vertex with id = " << std::to_string(g[edge.first].id());
