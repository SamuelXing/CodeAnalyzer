		...
		void doAction(const Scanner::ITokCollection* pTc) override
		{
			...
			if (nameIndex > 1 && (*pTc)[nameIndex - 1] == "::")
			{
				...
				*/
				...
				pFunctNode->parentTypeName_ = pClassNode->name_;
	...
	public:
		...
		void doAction(const Scanner::ITokCollection* pTc) override
		{
			...
			if (se[0] == "typedef")
			{
				...
				GrammarHelper::showParse("typdef declar", *pTc);
				return;
			}
			...
			if (GrammarHelper::isDataDeclaration(se))
			{
				...
				if (se.length() == 3)
				{
					declNode.typename_ = se[1];
				}
				if (se.length() == 5&&se[1]=="::") { declNode.typename_ = se[3]; }
				if (se.length() == 5 && se[2] == "=") { declNode.typename_ = se[1]; }
				pCurrNode->decl_.push_back(declNode);
	...
	*/
	...
	public:
		bool doTest(const Scanner::ITokCollection* pTc) override
		{
			...
			Scanner::SemiExp tc;
			tc.clone(*pTc);
			if (tc[0] == "enum" && (tc[tc.length() - 1] == "{" || tc[tc.length() - 1] == ";") && tc.length() > 2)
			{
	...
	public:
		HandleDefault(Repository* pRepos) : p_Repos(pRepos) {}
		...
		void doAction(const Scanner::ITokCollection* pTc) override
		{
			...
			ASTNode* pCurrNode = p_Repos->scopeStack().top();
			pCurrNode->type_ = "enum";
			pCurrNode->name_ = (*pTc)[pTc->find("enum")+1];
			pCurrNode->package_ = p_Repos->package();
			pCurrNode->path_ = p_Repos->currentPath();
			p_Repos->AST().typeMap()[pCurrNode->name_] = pCurrNode;
			GrammarHelper::showParse("using declar", *pTc);
			return;
		}
	};
