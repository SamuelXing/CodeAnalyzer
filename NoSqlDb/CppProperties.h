...
public:
	...
	T getConstValue() const{ return value; }
	friend bool operator==(const Property<T>& ls, const Property<T>& rs)
	{
		if ( ls.getConstValue() == rs.getConstValue())
		{
			return true;
		}
		return false;
...
//{
//public:
//	using SetterType = std::function<T(const std::vector<T>& vec)>;
//	using GetterType = std::function<T(const std::vector<T>& vec)>;
//
//	// innitialize
//	Property() :vec(std::vector<T>()) {}
//	Property(const std::vector<T>& t) :vec(t) {}
//	virtual ~Property()
//	{
//	}
//	virtual Property<std::vector<T>>& operator=(const std::vector<T>& t) 
//	{
//		vec = setter_(t);
//		return *this;
//	}
//	virtual operator const T() const { return getter_(vec); }
...
//private:
//	std::vector<T> vec;	
//	GetterType getter_ = [](const std::vector<T>& v) {return v};
//	SetterType setter_ = [](const std::vector<T>& v) {return v};
//};
