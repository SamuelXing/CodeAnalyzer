...
#include "../Graph/StrongComponent.h"
...
void showUsage()
{
  ...
  out << "\n    - x: persist found strong component to strongComponent.xml";
...
{ using namespace CodeAnalysis;
  ...
  try {
	...
	std::cout << "\n\n  Req #9: the unit test suite I provided is located on ~/TestFiles \n";
	TypeAnal ta;    // from here. doing my job
    ta.doTypeAnal();
	DependenceAnalysis::DepAnal da(ta.getTypeTable());
	da.doDepAnal();
	GraphLib::Graph<std::string, std::string> graph;
	DependenceAnalysis::DependcyTable deptb = da.getDepTable(); 
	graph.initialize(deptb); 
	GraphLib::Display<std::string, std::string>::show(graph);
	std::cout << "\n\n  Strong Component --- Req #6: find strong components in the dependency graph \n ==============================\n";
	StrongComponents scc;
	graph.SCC(scc);
	std::cout << "persist found strong component to "<< exec.SCxml_<< "\n";
	std::string xml = scc.persist();
	scc.save(xml, exec.SCxml_);}      /*  meet req #8: supplying default xml results file */
